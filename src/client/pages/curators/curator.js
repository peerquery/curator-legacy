...
$( document ).ready(function() {
    ...
    var curator_account = url.substring(9);
	
    document.title = '@' + curator_account;
	
    $('#user_img').attr('src', 'https://steemitimages.com/u/' + curator_account + '/avatar');
    $('#curator_account').text('@' + curator_account);
	
	
    $('#user_steemit').attr('href', 'https://steemit.com/@' + curator_account);
    $('#user_busy').attr('href', 'https://busy.org/@' + curator_account);
    $('#user_dtube').attr('href', 'https://d.tube/#!/c/' + curator_account);
    $('#user_dlive').attr('href', 'https://dlive.io/@' + curator_account);
    $('#user_utopian').attr('href', 'https://utopian.io/@' + curator_account);
    $('#user_steepshot').attr('href', 'https://alpha.steepshot.io/@' + curator_account);
    $('#user_steempeak').attr('href', 'https://steempeak.com/@' + curator_account);
    $('#user_dsound').attr('href', 'https://dsound.audio/#!/@' + curator_account);
    $('#user_musing').attr('href', 'https://musing.io/profile/' + curator_account);
    $('#user_steemhunt').attr('href', 'https://steemhunt.com/author/' + curator_account);
    $('#user_peerquery').attr('href', 'https://www.peerquery.com/@' + curator_account);
    ...
    function approved() {
        ...
        $.get('/api/approved/curator/' + curator_account, function(data, status){
    ...
    function voted() {
        ...
        $.get('/api/voted/curator/' + curator_account, function(data, status){
    ...
    function rejected() {
        ...
        $.get('/api/rejected/curator/' + curator_account, function(data, status){
    ...
    function create_items(data) {
        ...
        if (modal_mode == 'ignored' || modal_mode == 'lost') {
            ...
            description.innerHTML = '<small>Authored by <a target=\'_blank\' href=\'/user/' + data.author + '\'><b>@' + data.author + '</b></a>, curated as - <em>' + rate_name(data.rate) + '</em></small>';
        }
    ...
    (async () => {
        ...
        $.get('/api/curator_stats/' + curator_account, function(data, status){
            //console.log(data);
    ...
    (async () => {
        ...
        $.get('/api/curator_rep/' + curator_account, function(data, status){
            //console.log(data);
			
            var rep = get_ratings(data[0].points);
